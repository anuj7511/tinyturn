...
Writes to a NEW experiment directory (`whisper_tiny_speech_aligned_contract`) rather than overwriting
`experiments/whisper_tiny_baseline/` -- per Phase-2 8d, "the previous results for both remain historical
baselines only," and per the implementation rules, prior experiment directories are never
overwritten.

Same hyperparameters as the original whisper_tiny_baseline run (context_s=4.0, 2 epochs, batch_size=8,
...
retrain (scripts/retrain_b1_1s_speech_aligned_contract.py).

Usage:
  python scripts/retrain_a0_speech_aligned_contract.py
"""
import sys
...
def main():
    ...
    out_dir = Path("experiments") / "whisper_tiny_speech_aligned_contract"
