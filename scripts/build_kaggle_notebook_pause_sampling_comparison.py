...
present in the already-uploaded tar), and reproduces train_pause_sampling_comparison.py's orchestration
...
step9_orchestration_src = read("scripts/train_pause_sampling_comparison.py")
...
assert m, "couldn't locate the constants+function block in train_pause_sampling_comparison.py"
...
# The regex above only captured this slice, not train_pause_sampling_comparison.py's top-level imports
...
Reproduces `scripts/train_pause_sampling_comparison.py`'s reporting logic unmodified."""
))
...
TARGET_RECALLS = [0.90, 0.95]  # used by the comparison cell below (at_matched_recall_v2) -- was
                                ...
                                # extracts reusable functions from train_pause_sampling_comparison.py
