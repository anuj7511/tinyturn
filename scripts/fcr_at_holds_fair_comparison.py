...
from experiments/context_ablation_mel_trajectory_1s/metrics.json) -- an apples-to-oranges comparison, since a lower threshold
...
def main():
    ...
    baseline_threshold = json.load(open("experiments/context_ablation_mel_trajectory_1s/metrics.json"))["threshold"]
    p1_threshold = json.load(open("experiments/pause_events_original_recipe/metrics.json"))["threshold"]

    baseline = TinyTurnModel(n_mels=40, trajectory_dim=len(TRAJECTORY_NAMES))
    baseline.load_state_dict(torch.load("experiments/context_ablation_mel_trajectory_1s/checkpoint.pt", map_location=device))
    p1 = TinyTurnModel(n_mels=40, trajectory_dim=len(TRAJECTORY_NAMES))
    p1.load_state_dict(torch.load("experiments/pause_events_original_recipe/checkpoint.pt", map_location=device))
    ...
    with open("experiments/pause_events_original_recipe/fcr_at_holds_fair_compare.json", "w") as f:
        json.dump(out, f, indent=2)
    print("saved experiments/pause_events_original_recipe/fcr_at_holds_fair_compare.json")
