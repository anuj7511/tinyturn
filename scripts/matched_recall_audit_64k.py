...
same method as matched_recall_audit.py (threshold selected on CALIB's recall curve, evaluated on
VAL only -- never selected and evaluated on the same split). Kept as a separate script/output rather
than folding into matched_recall_audit.py's CHECKPOINTS dict so re-running it doesn't re-audit
...
Usage:
  python scripts/matched_recall_audit_64k.py
"""
import json
import sys
...
from scripts.matched_recall_audit import audit_one, CACHE_DIR, CONTEXT_S
