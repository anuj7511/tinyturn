...
before. Combines two existing pieces rather than re-deriving either:
  ...
  - Boundary computation (canonical / alt-threshold / Silero) on the full 1,600-clip val split,
    exactly as in `run_8f_vad_boundary_diagnostic_v2.py` (same alt-threshold formula, same Silero
  ...
  - Ground-truth-conditioned introduced-error metrics from `run_metric_audit.py`
...
same convention as 8f-v2 and run_metric_audit.py -- never recomputed here.
...
Usage:
  python scripts_part3/run_8f_vad_boundary_diagnostic_b1_64k.py
  python scripts_part3/run_8f_vad_boundary_diagnostic_b1_64k.py --refresh-boundaries  # force recompute
"""
import argparse
import json
import sys
...
from scripts_part3.run_metric_audit import (
...
# Same constants as run_8f_vad_boundary_diagnostic_v2.py -- deliberately a different alt-threshold
