...
Steps 1-3 of the brief's 5-step remediation:
  1. A/B/C boundaries for D2 training audio -- precomputed separately by
     scripts_part3/precompute_train_boundary_augmentation.py into
...
Usage:
  python scripts_part3/run_8g_remediation_retrain_a0.py [--force]
"""
import sys
...
def main():
    if not TRAIN_BOUNDARY_AUGMENTATION_PATH.exists():
        print(f"ERROR: {TRAIN_BOUNDARY_AUGMENTATION_PATH} not found -- run "
              f"scripts_part3/precompute_train_boundary_augmentation.py first (full train split, "
              f"no --limit).")
        sys.exit(1)
    ...
    print(
        ...
        f"  python scripts_part3/run_8e_padding_counterfactual.py {OUT_DIR}\n"
        f"  python scripts_part3/run_8f_vad_boundary_diagnostic_v2.py {OUT_DIR}\n"
        f"  python scripts_part3/run_8g_qualify_teacher_v2.py {OUT_DIR}\n"
