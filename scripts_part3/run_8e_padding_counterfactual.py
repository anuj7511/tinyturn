...
produced by run_8d_retrain_a0.py): take val-split clips short enough that build_example needs
...
Usage:
  python scripts_part3/run_8e_padding_counterfactual.py
"""
import json
import sys
...
def main():
    if not (EXP_DIR / "checkpoint.pt").exists():
        print(f"ERROR: {EXP_DIR / 'checkpoint.pt'} not found -- run run_8d_retrain_a0.py first.")
        sys.exit(1)
